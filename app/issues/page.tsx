  ...
  return (
    <ProtectedRoute>
      <div className="flex h-screen bg-gray-100 font-sans antialiased overflow-hidden">
        ...
        {/* --- ১. সাইডবার --- */}
        <aside className={`fixed inset-y-0 left-0 z-50 w-64 bg-slate-900 text-white transform transition-transform duration-300 ease-in-out lg:translate-x-0 lg:static lg:inset-auto ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
          <div className="flex items-center justify-between p-5 border-b border-slate-800">
            <span className="text-xl font-bold tracking-wider text-blue-400">🛡️ IssueTracker</span>
          ...
          <nav className="p-4 space-y-2">
            <a href="#" className="flex items-center space-x-3 px-4 py-3 bg-blue-600 rounded-lg text-white font-medium">
          ...
          <div className="absolute bottom-0 w-full p-4 border-t border-slate-800 bg-slate-950">
            <div className="flex items-center justify-between">
              ...
              <button onClick={logout} className="p-2 bg-red-600 hover:bg-red-700 rounded-lg text-xs font-semibold transition-colors">
                Logout
              </button>
            </div>
          </div>
        ...
        {isSidebarOpen && <div onClick={() => setIsSidebarOpen(false)} className="fixed inset-0 z-40 bg-black/40 lg:hidden" />}
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          ...
          <header className="flex items-center justify-between bg-white border-b border-gray-200 px-4 sm:px-6 py-4 shadow-sm">
            <div className="flex items-center space-x-3">
              <button onClick={() => setIsSidebarOpen(true)} className="p-2 -ml-2 text-gray-500 hover:text-gray-700 rounded-lg lg:hidden">
          ...
          <main className="flex-1 overflow-x-hidden overflow-y-auto bg-gray-50 p-4 lg:p-8">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 mb-6">
              <div className="flex flex-col lg:flex-row gap-4 justify-between items-start lg:items-center">
                <h2 className="text-base font-bold text-gray-700">Filter & Sort Board</h2>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
              {loadingIssues ? (
                ...
                <div className="text-center py-16 text-gray-500">
                  ...
                  <p className="text-sm mt-1">Try changing your filters or create a new issue.</p>
                </div>
              ) : (
                  ...
                  <table className="w-full text-left border-collapse">
                    <thead>
                      <tr className="bg-gray-50 border-b border-gray-200 text-xs font-semibold text-gray-500 uppercase tracking-wider">
                        ...
                        <th className="px-6 py-4 hidden md:table-cell">Reported By</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-gray-200 text-sm text-gray-700">
                      {issues.map((issue) => (
                        <tr key={issue.id} className="hover:bg-gray-50 transition-colors">
                          <td className="px-6 py-4 font-bold text-gray-400">#{issue.id}</td>
                          <td className="px-6 py-4">
                            <div className="font-semibold text-gray-900 break-words max-w-xs sm:max-w-md">{issue.title}</div>
                            <div className="text-xs text-gray-500 mt-1 line-clamp-1 max-w-xs sm:max-w-md">{issue.description}</div>
                            <div className="text-[11px] text-gray-400 md:hidden mt-2 flex flex-wrap gap-x-2">
                              <span>👤 {issue.reporter?.name || 'Unknown'} ({issue.reporter?.role})</span>
                              <span>• {new Date(issue.created_at).toLocaleDateString()}</span>
                            </div>
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap">
                            <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-semibold uppercase ${issue.type === 'bug' ? 'bg-red-50 text-red-700 border border-red-200' : 'bg-indigo-50 text-indigo-700 border border-indigo-200'}`}>
                          ...
                          <td className="px-6 py-4 hidden md:table-cell whitespace-nowrap">
                            <div className="font-medium text-gray-900">{issue.reporter?.name || 'Unknown'}</div>
                            <div className="text-xs text-gray-400 capitalize">{issue.reporter?.role || 'N/A'}</div>
                          </td>
                        </tr>
                      ))}
                    </tbody>
